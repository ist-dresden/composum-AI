...
[Deepl Translator](https://www.deepl.com/translator) . It's quick and easy to try that on your own site - try it
out on your own site and let us know what you think and how it works for you! Please contact us on any trouble!
...
- Unlike most existing machine translation solutions for AEM we collect all the texts of one page, experience fragment
  ...
  translation request to ChatGPT. Thus, it take the context of the text into account and translate it accordingly.
...
Please notice that we currently assume the translations to be live copies of the original, not just language copies, as
described in the next section, since that allows easily updating the structure.
...
very close to each other in content and structure and mostly the language is different. An interesting point about the
current prototype is that it currently requires that the other languages are live copies of their primary language.
Thus, changes in the master language can be rolled out to the other languages, and the automatic translation takes care
of
...
exceptions in an OSGI configuration "Composum AI Autotranslate Configuration" (see [configuration reference](..
/autogenerated/osgiconfigurations.md) ).

## Usage of the POC UI

Since this is a proof of concept, there is a very simple POC style UI. The entry point for the translation is the 
URL in the author server
...
- the live copy has either to have a path like .../de/.. the language can be determined from, or set to the desired
  language
- as rollout configuration there should be a configuration like the standard rollout config that updates the page,
  and then (after that!) the "Composum AI Autotranslate POC" rollout configuration

Then the pages will be transparently be translated on each rollout.

For the initial live copy creation we recommend to set up the live copy with the right language setting, try the
tanslation using the workflow or POC UI on a few pages, and only then add the rollout configuration.
