...
echo We use -cn with aigenpipeline since this works with OpenAI but Claude seems to have some trouble with this for a yet unknown reason
...
for file in $(fgrep -l -R --include="*.java" '@ObjectClassDefinition'); do
  ...
  aigenpipeline -cn -p $DIR/osgiconfigurations.prompt $file -o $mdname
done
