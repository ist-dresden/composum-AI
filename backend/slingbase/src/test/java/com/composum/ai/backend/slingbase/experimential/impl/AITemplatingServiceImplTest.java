...
public class AITemplatingServiceImplTest {
    ...
    public void testCollectTexts_withValidReplacements() {
        ...
        ec.checkThat(texts.keySet(), allOf(hasItem("#TEXT001"), hasItem("#ID1"), hasItem("#TEXT002"), hasItem("informationally003")));

        ec.checkThat(texts.get("#TEXT001"), is("single sentence invitation to check out the product"));
        ec.checkThat(texts.get("#ID1"), is("name of the product"));
        ec.checkThat(texts.get("#TEXT002"), is("<p><strong>markdown list of key features</strong></p>"));
        ec.checkThat(texts.get("informationally003"), is("Key Features"));

        ec.checkThat(ids.size(), is(3));
        ec.checkThat(ids.get("#TEXT001"), is(r1));
        ec.checkThat(ids.get("#ID1"), is(r2));
        ec.checkThat(ids.get("#TEXT002"), is(r3));
    }
